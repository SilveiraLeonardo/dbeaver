...
public class OracleChangeUserPasswordManager implements DBAUserPasswordManager {
    ...
    public void changeUserPassword(DBRProgressMonitor monitor, String userName, String newPassword, String oldPassword) throws DBException {
        ...
        try (JDBCSession session = DBUtils.openMetaSession(monitor, dataSource, "Change user password")) {
            ...
            try (PreparedStatement pstmt = session.prepareStatement(sql)) {
                pstmt.setString(1, userName);
                pstmt.setString(2, CommonUtils.notEmpty(newPassword));
                pstmt.setString(3, CommonUtils.notEmpty(oldPassword));
                pstmt.executeUpdate();
            }
